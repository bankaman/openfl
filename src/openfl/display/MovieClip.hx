...
class MovieClip extends Sprite #if (openfl_dynamic && haxe_ver < "4.0.0") implements Dynamic<DisplayObject> #end {
	...
	**/
    ...
    @:noCompletion private static function __init__() {
        __useParentFPS = true;
        untyped #if haxe4 js.Syntax.code #else __js__ #end("/// #if (typeof ENV === 'undefined' || (!ENV['swflite-parent-fps'] && !ENV['swf-parent-fps'])) && (typeof swf_parent_fps === 'undefined' || !swf_parent_fps) && (typeof swflite_parent_fps === 'undefined' || !swflite-parent-fps) && (typeof defines === 'undefined' || (!defines['swf-parent-fps'] && !defines['swflite-parent-fps']))");
        __useParentFPS = false;
        untyped #if haxe4 js.Syntax.code #else __js__ #end("/// #endif");
	...
	**/
    ...
    @:noCompletion private function __updateSymbolNew(targetFrame:Int):Void {
        if (__symbol == null || __currentFrame == __lastFrameUpdate) {
            return;
        }
        __updateFrameLabel();

        trace('frame-data:', __symbol.frames);
    ...
    @:noCompletion private function __updateSymbol(targetFrame:Int):Void {
        if (__symbol != null && __currentFrame != __lastFrameUpdate) {
            ...
            if ((__currentFrame + 1) == targetFrame) {
                startFrame = __currentFrame;
            }

            for (i in startFrame...targetFrame) {
                frame = i + 1;
        ...
        for (i in 0...__symbol.frames.length) {
            ...
            if (frameData.script != null) {
                ...
                try {
					...
					#elseif js
                    var script = untyped #if haxe4 js.Syntax.code #else __js__ #end("eval({0})", "(function(){" + frameData.scriptSource + "})");
